...
function showOptionNomBalise(){
    ...
    //
    //if(nomBalise.search(/,/) != -1) {
	//	alert("Veuillez saisir qu'une balise");
	//	baliseUnSelectAll();
		...
		$.ajax({
			...
			success: function (response) {
				if (response) {
					...
					if (confirm(getTextConfirmChangerNumeroBalise+": " + numeroAppel + " -> " + numeroAppelNew)) {
						$.ajax({
							url: '../option/optionoknumero.php',
							type: 'GET',
							data: "InternationalFormat=" + InternationalFormat + "&numeroAppelNew=" + numeroAppelNew + "&Id_Tracker=" + Id_Tracker +
							"&nomDatabaseGpw=" + nomDatabaseGpw + "&ipDatabaseGpw=" + ipDatabaseGpw + "&timezone=" + timezone,
							success: function (response) {
								if (response) {
									//alert(response);
									document.body.className = "loading";
