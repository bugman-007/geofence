	...
	if (Id_Tracker==""){
		...
		$.ajax({
			...
			success: function (response2) {
				...
				if (response2) {
					...
					if (nbreLigne) {
						for (var i = 0; i < nbreLigne; i++) {
							...
							{
								if(( versionBaliseGlobal == "53" )||( versionBaliseGlobal == "3370" )||( versionBaliseGlobal == "7003" )||( versionBaliseGlobal == "7201" ))	// CUBE, NEO & SOLO
				...
				if(( versionBaliseGlobal == "20" )||( versionBaliseGlobal == "3370" )||( versionBaliseGlobal == "7003" )||( versionBaliseGlobal == "7201" ))	// TELTO , NEO & SOLO
				{
		...
		$.ajax({
			...
			success: function (response) {
				if (response) {
                    ...
                    coordPosStatut = response.substring(response.indexOf('Pos_Statut') + 11, response.indexOf('Pos_Vitesse'));
                    coordPosVitesse = Math.round(parseInt(response.substring(response.indexOf('Pos_Vitesse') + 12, response.indexOf('Pos_Direction'))));
					...
					Pos_Key = response.substring(response.indexOf('Pos_Key:') + 8, response.indexOf('Statut2:'));
					Statut2 = response.substring(response.indexOf('Statut2:') + 8, response.indexOf('BattInt:'));
					BattInt = response.substring(response.indexOf('BattInt:') + 8, response.indexOf('BattExt:'));
					BattExt = response.substring(response.indexOf('BattExt:') + 8, response.indexOf('Alim:'));
					Alim =  response.substring(response.indexOf('Alim:') + 5, response.indexOf('TypeServer:'));
					TypeServer = response.substring(response.indexOf('TypeServer:') + 11, response.indexOf('Icone:'));
